...
class SynthObs():
    ...
    @staticmethod
    def return_swddata(h, vs, vpvs=1.73, ra=0, methods=3, pars=dict(), x=None):
        ...
        for i, target in enumerate(targets):
            xmod, ymod = target.moddata.plugin.run_model(
                h=h, vp=vp, vs=vs, ra=ra, rho=rho,methods=methods)
    ...
    @staticmethod
    def save_model(h, vs, vpvs=1.73, ra=0, vs_methods=3, outfile=None):
        ...
        rho = vp * 0.32 + 0.77
      	...
      	if type(ra) == np.float or type(ra) == np.int:
      	    ra = np.ones(len(vs)) * ra
            ra = np.array(ra)
      	vsv = Model.get_vsv_vsh(vs, ra, vs_type='vsv', methods = vs_methods)
      	vsh = Model.get_vsv_vsh(vs, ra, vs_type='vsh', methods = vs_methods)
