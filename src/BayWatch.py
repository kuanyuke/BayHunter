...
class BayWatcher(object):
    ...
    def __init__(self, configfile, capacity=100, address='127.0.0.1',
        ...
        self.modellength = int(self.priors['layers'][1] + 1) * 3
        self.vs_step = np.ones((capacity, self.modellength)) * np.nan
        self.ra_step = np.ones((capacity, self.modellength)) * np.nan
    ...
    def init_plot(self):
        ...
        if self.plot_ra:
            ...
            ypost = 6
            colspan1 = 3
            ax6 = plt.subplot2grid((10, ygrid), (0, 4), rowspan=10, colspan=1)  # radial anisotropy
        else:
            ygrid = 8
            ypost = 4
            colspan1 = 3
        ax1 = plt.subplot2grid((10, ygrid), (0, 0), rowspan=10, colspan=colspan1)  # vel-dep
        inax = inset_axes(ax1, width="50%", height="5%", loc=3)
        ax2 = plt.subplot2grid((10, ygrid), (0, ypost), rowspan=2, colspan=4)  # like
        ax3 = plt.subplot2grid((10, ygrid), (4, ypost), rowspan=3, colspan=4)  # rf
        ax4 = plt.subplot2grid((10, ygrid), (7, ypost), rowspan=3, colspan=4)  # disp
        ax5 = plt.subplot2grid((10, ygrid), (2, ypost), rowspan=2, colspan=4)  # noise
        ...
        self.axes[0].set_xlabel('average Vs in km/s')
        ...
        if self.plot_ra:
            ...
            self.modelline, = self.axes[6].plot(np.nan, np.nan, color='k', lw=0.7)
            colors = np.arange(self.capacity)
            segments = [np.column_stack([x, y])
                        for x, y in zip(self.ra_step, self.dep_step)]

            lc = LineCollection(segments, cmap='plasma_r')
            lc.set_array(np.asarray(colors))

            self.racollection = self.axes[6].add_collection(lc)
            self.racollection.set_linewidths(0.7)

            self.axes[6].set_ylim(self.priors['z'])
            self.axes[6].invert_yaxis()
            self.axes[6].set_xlabel('Radial Anisotropy in %')
            self.axes[6].grid(color='gray', ls=':')

            if type(self.priors['ra']) in [tuple,list,np.array]:
                self.axes[6].set_xlim(self.priors['ra'][0]-2,self.priors['ra'][1]+2)

            elif type(self.priors['ra']) == float or type(self.priors['ra']) == int:
                self.axes[6].set_xlim(self.priors['ra']-2, self.priors['ra']+2)
    ...
    def update_chain(self):
        ...
        for i, model in enumerate(self.modelmatrix):
            ...
            if ~np.isnan(model[0]):
                vp, vs, ra, dep = Model.get_stepmodel(model, vpvs=self.vpvss[i], mantle=self.mantle)
                ...
                if self.plot_ra:
                    self.ra_step = np.roll(self.ra_step, -1, axis=0)  # rolling up models
                    ra = nantmp[:ra.size] = ra
                    self.ra_step[-1][:ra.size] = ra
        ...
        vp, vs, ra, h = Model.get_vp_vs_h(lastmodel, vpvs=lastvpvs, mantle=self.mantle)
        ...
        # # create LineCollection and update radial anisotropy
        if self.plot_ra:
            segments = [np.column_stack([x, y])
                        for x, y in zip(self.ra_step, self.dep_step)]
            self.racollection.set_segments(segments)
    ...
    def update_models(self, model, vpvs):
        ...
        vp, vs, ra, dep = Model.get_stepmodel(model, vpvs=vpvs, mantle=self.mantle)
        ...
        if self.plot_ra:
            ...
            nantmp = np.ones(self.modellength) * np.nan
            nantmp[:ra.size] = ra
            self.ra_step[-1] = nantmp
            
        vp, vs, ra, h = Model.get_vp_vs_h(model, vpvs=vpvs, mantle=self.mantle)
        ymod = self.compute_synth(h, vs, vp, ra)
