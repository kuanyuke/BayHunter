...
class ModeledData(object):
    ...
    def calc_synth(self, h, vp, vs, **kwargs):
        ...
        if self.plugin.ref in rf_targets:
            self.x, self.y = self.plugin.run_model(h, vp, vs, rho=rho, **kwargs)
        else:
            ra = kwargs.pop('ra') 
            methods = kwargs.pop('methods')  
            self.x, self.y = self.plugin.run_model(h, vp, vs, ra, rho=rho, methods= methods, **kwargs)            
...
class JointTarget(object):
    ...
    def evaluate(self, h, vp, vs, ra, methods, noise, **kwargs):
        ...
        for n, target in enumerate(self.targets):
            target.moddata.calc_synth(h=h, vp=vp, vs=vs, ra =ra, rho=rho, methods=methods, **kwargs)
